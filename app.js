...
function emptyForm() { return { tab: 'species', name: '', text: '', types: [], typeMode: 'all', abilities: [], moves: [], stats: [{ stat: 'spe', op: '>=', val: '' }], formes: { base: true, mega: true }, regs: [], matchups: [{ rel: 'weak', type: '' }], cats: new Set(), mnums: [{ stat: 'bp', op: '>=', val: '' }], props: [], target: 'any', lb: '', icats: [], view: 'grid', order: '', dir: '', also: [] }; }
function buildQuery(fs) {
  ...
  { const intro = fs.regs.filter(x => x.v.startsWith('new:') && !x.neg); const rest = fs.regs.filter(x => !(x.v.startsWith('new:') && !x.neg)); if (intro.length === 1) t.push(intro[0].v); else if (intro.length > 1) t.push('(' + intro.map(x => x.v).join(' or ') + ')'); rest.forEach(x => { if (x.v === 'r:' + IDX.currentReg.toLowerCase() && !x.neg) return; t.push((x.neg ? '-' : '') + x.v); }); }
...
function queryToForm(q) {
  ...
  for (const it of items) {
    ...
    if (it.type === 'or' && it.items.every(x => x.type === 'term' && x.op === ':' && !x.isRegex)) {
      ...
      if (fields.size === 1 && fields.has('new') && vals.every(v => IDX.regs.some(r => norm(r).replace(/ /g, '') === norm(v).replace(/ /g, '')))) { vals.forEach(v => fs.regs.push({ v: 'new:' + IDX.regs.find(r => norm(r).replace(/ /g, '') === norm(v).replace(/ /g, '')).toLowerCase(), neg: false })); continue; }
    ...
    if (['r', 'new', 'banned', 'restricted', 'removed'].includes(f) && tm.op === ':') { const rv = IDX.regs.find(r => norm(r).replace(/ /g, '') === vn.replace(/ /g, '')); if (rv) { fs.regs.push({ v: f + ':' + rv.toLowerCase(), neg }); continue; } }
...
function advForm() {
  ...
  const regRow = row('reg', 'Regulation', band(tokens('regs')), `Legal in, Introduced in, Banned in or Removed in a regulation, each with IS / NOT. Two “Introduced in” picks mean either; everything else must all hold. No pick = the current regulation, ${esc(IDX.currentReg)}. Regulations so far: ${IDX.regs.join(' → ')}.`);
...
function addToken(key, raw) {
  ...
  else if (key === 'regs') { const items = REG_ITEMS(); const hit = items.find(([id, l]) => norm(l).replace(/ current/, '').trim() === norm(v).replace(/ current/, '').trim() || id === v.toLowerCase().replace(/\s+/g, '')) || (IDX.regs.some(r => norm(r).replace(/ /g, '') === norm(v).replace(/ /g, '')) ? items.find(([id]) => id === 'r:' + IDX.regs.find(r => norm(r).replace(/ /g, '') === norm(v).replace(/ /g, '')).toLowerCase()) : null); if (!hit) return false; val = hit[0]; }
