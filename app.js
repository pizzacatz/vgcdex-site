...
function typePicker(i, val) { return `<div class="tok-wrap single typepick" data-f="type" data-val="${esc(val)}"><div class="tokens">${val ? `<span class="tok"><span class="chip type-${esc(val)}">${esc(val)}</span><button type="button" class="tok-x" data-mtypex="${i}" aria-label="Remove ${esc(val)}">×</button></span>` : ''}<input type="text" class="form-input tok-in" data-tkin="mtype" data-single="1" data-mtypein="${i}" placeholder="${val ? '' : 'Type…'}" autocomplete="off" autocapitalize="off" spellcheck="false"></div><div class="tok-menu" data-menu="mtype" hidden></div></div>`; }
...
function bindForm() {
  ...
  f.addEventListener('click', ev => {
    ...
    const mx = ev.target.closest('[data-mtypex]'); if (mx) { setMatchType(mx.dataset.mtypex, ''); return; }
