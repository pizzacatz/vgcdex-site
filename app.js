...
const HOME_TEAM = [
  ...
  { file: 'tyranitar-mega', slug: 'tyranitarmega', w: 454, row: 'front', x: -0.69, y: 80, rot: -3 },
  { file: 'indeedee-male', slug: 'indeedee', w: 251, row: 'front', x: 0.41, y: 74, rot: 2 },
  { file: 'corviknight', slug: 'corviknight', w: 336, row: 'back', x: 0.96, y: 10, rot: 4 },
  { file: 'sneasler', slug: 'sneasler', w: 300, row: 'front', x: 1.46, y: 84, rot: 6 },
];
...
function itemCard(e) { return `<a class="scard icard" href="${plink(e)}" data-nav><div class="dtop"><div class="art">${e.sprite ? `<img src="${e.sprite}" alt="" loading="lazy">` : ''}${e.reg === IDX.currentReg ? `<span class="newreg">New<span class="nr-r"> in ${esc(e.reg)}</span></span>` : (e.legality[IDX.currentReg] === 'banned' ? '<span class="newreg banned">Banned</span>' : '')}</div><div class="icats">${e.cats.map(c => `<span class="chip neutral">${esc(c)}</span>`).join('')}</div></div><div class="nm">${esc(e.name)}</div></a>`; }
...
function detail(d) {
  ...
  if (e.kind === 'species') {
    ...
    return `<section class="wrap page">${back}<div class="page-grid"><div class="page-main"><h1>${esc(e.name)}${e.is_mega ? ' <span class="badge megab">Mega</span>' : ''}</h1><div class="sphead"><div class="spinfo"><div class="chips big">${e.types.map(typeChip).join('')}</div>
      ...
      <h3>Abilities</h3><ul class="plain">${e.abilitySlugs.map((s, i) => { const a = IDX.ents.find(x => x.kind === 'ability' && x.slug === s); return `<li><a href="${a ? plink(a) : '#'}" data-nav><b>${esc(e.abilities[i])}</b></a> <span class="muted">${esc(a ? a.raw.short_desc || '' : '')}</span></li>`; }).join('')}</ul>
...
function openMenu(input) { if (MENU_MUTE) return; nudgeIntoView(input); if (input.dataset.acsub || ((input.dataset.tkin === 'moves' || input.dataset.tkin === 'abilities') && looksExpr(input.value))) return openSubMenu(input); const key = input.dataset.tkin; const menu = input.closest('.tok-wrap').querySelector('.tok-menu'); const rows = menuRows(key, input.value); MENU = { key, rows, hi: rows.length && input.value ? 0 : -1, input, menu };
  ...
  menu.innerHTML = (grouped ? '' : `<div class="tok-menu-group">${esc(TK_META()[key].group)}</div>`) + rows.slice(0, 400).map(([v, l, g, note], i) => { let h = ''; if (grouped && g !== lastG) { h = `<div class="tok-menu-group">${esc(g)}</div>`; lastG = g; } return h + `<div class="tok-menu-row ${i === MENU.hi ? 'hi' : ''}" data-pick="${esc(v)}" data-i="${i}" title="${esc(note || '')}">${key === 'types' || key === 'mtype' ? pill(v) : esc(l)}${note ? ' <span class="muted">ⓘ</span>' : ''}</div>`; }).join('') + (rows.length > 400 ? `<div class="tok-menu-empty">…keep typing to narrow the list</div>` : ''); menu.hidden = false; }
...
function pick(input, v) { if (input.dataset.acsub || MENU.key === 'sub' || MENU.key === 'main') return pickSub(input, v); if (input.dataset.mtypein !== undefined) return void setMatchType(input.dataset.mtypein, v); const key = input.dataset.tkin; if (input.dataset.single) { input.value = v; closeMenu(); readForm(); return; } if (addToken(key, v)) { closeMenu(); rerenderTokens(key); const nin = $(`[data-tkin="${key}"]`); if (nin) nin.focus(); } }
...
// looks like the dropdown beside it, but you can type to filter; the choice shows as a chip
function typePicker(i, val) { return `<div class="tok-wrap typesel" data-f="type" data-val="${esc(val)}"><div class="selbox" data-selbox>${val ? `<span class="chip type-${esc(val)}">${esc(val)}</span>` : ''}<input type="text" class="tok-in" data-tkin="mtype" data-single="1" data-mtypein="${i}" placeholder="${val ? '' : 'Type…'}" aria-label="Matchup type" autocomplete="off" autocapitalize="off" spellcheck="false"></div><div class="tok-menu" data-menu="mtype" hidden></div></div>`; }
// commit or clear one matchup row's type, then keep an empty row at the end of the repeater
function setMatchType(i, v) { const row = $(`[data-row="matchups"][data-i="${i}"]`); if (!row || !FS.matchups[i]) return false; const t = norm(String(v || '')); if (v !== '' && !IDX.types.includes(t)) return false;
  FS.matchups[i] = { rel: row.querySelector('[data-f=rel]').value, type: v === '' ? '' : t };
  closeMenu(); row.outerHTML = matchRow(FS.matchups[i], i); readForm(); ensureDupRow('matchups');
  const nin = $(`[data-mtypein="${i}"]`); if (nin && v !== '') nin.blur(); else if (nin) { MENU_MUTE = true; nin.focus(); setTimeout(() => { MENU_MUTE = false; }, 0); } return true; }
...
function matchRow(r, i) { return `<div class="band dup" data-row="matchups" data-i="${i}"><select class="form-input auto small-select" data-f="rel">${MATCH_OPTS.map(([v, l]) => `<option value="${v}" ${v === r.rel ? 'selected' : ''}>${l}</option>`).join('')}</select>${typePicker(i, r.type)}</div>`; }
function advForm() {
  ...
  if (T === 'species') body = typesRow
    + row('ability', 'Abilities', band(tokens('abilities')), 'Any of the Pokémon’s abilities; “NOT” excludes. A regex such as <code>/weather/</code> matches ability text; <code>o:immune</code> works too. See <a href="?guide=1#subquery" data-nav>the syntax guide</a>.')
...
let IGNORED = [];
const TOK_FIELD = { moves: 'Learns', abilities: 'Abilities', types: 'Types', props: 'Properties', icats: 'Item category', crit: 'Formes', mtype: 'Matchups' };
function flushInput(input) { if (!input) return true;
  if (input.dataset.mtypein !== undefined) { const v = input.value.trim(); return !v || setMatchType(input.dataset.mtypein, v); }
  if (input.dataset.single) return true; const v = input.value.trim(); if (!v) return true;
...
function bindForm() {
  ...
  f.addEventListener('input', ev => { const t = ev.target; if (t.classList.contains('tok-in')) { if (t.dataset.mtypein !== undefined) t.closest('.tok-wrap').classList.toggle('typing', !!t.value); openMenu(t); if (t.dataset.single) readForm(); return; } readForm(); const row = t.closest('[data-row]'); if (row && t.dataset.f === 'val' && t.value !== '') ensureDupRow(row.dataset.row); });
  f.addEventListener('focusin', ev => { const t = ev.target; if (t.classList && t.classList.contains('tok-in')) openMenu(t); });
  f.addEventListener('focusout', ev => { const t = ev.target; if (!(t.classList && t.classList.contains('tok-in')) || MENU_HOLD) return;
    closeMenu();
    if (t.dataset.mtypein !== undefined) { const v = t.value.trim(); if (v && !setMatchType(t.dataset.mtypein, v)) { t.value = ''; t.closest('.tok-wrap').classList.remove('typing'); } return; }
    flushInput(t); });
