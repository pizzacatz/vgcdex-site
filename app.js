...
function advForm() {
  ...
  const regulationRow = row('reg', 'Regulation', `<div id="regs-rows">${fs.regs.map(regRow).join('')}</div>`, `Each row is one condition; choosing a regulation adds another row. Several “Introduced in” rows mean any of them; everything else must all hold. No condition = the current regulation, ${esc(IDX.currentReg)}. Regulations so far: ${IDX.regs.join(' → ')}.`);
