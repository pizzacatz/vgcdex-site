...
function advForm() {
  ...
  const sorts = T === 'species' ? SORTS.filter(([v]) => !['bp', 'acc', 'pp', 'prio', 'cat', 'count'].includes(v)) : T === 'move' ? SORTS.filter(([v]) => ['', 'name', 'bp', 'acc', 'pp', 'prio', 'type', 'cat'].includes(v)) : T === 'ability' ? SORTS.filter(([v]) => ['', 'name', 'count'].includes(v)) : SORTS.filter(([v]) => ['', 'name'].includes(v));
